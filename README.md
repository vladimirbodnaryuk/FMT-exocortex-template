...
2. Заменяет 7 плейсхолдеров (`your-username`, `/mnt/d/Git` и др.)
...
1. Замените `your-username` на ваш GitHub username во всех файлах
2. Замените `/mnt/d/Git` на путь к рабочей директории (напр. `~/Github`)
3. Замените `/home/vb` на домашнюю директорию (значение `$HOME`)
4. Замените `-mnt-d-Git` на путь через дефисы (напр. для `~/Github` → `-Users-yourname-Github`)
5. Замените `4` на час запуска стратега в UTC (напр. `4` для 7:00 MSK)
6. Замените `4:00 UTC` на описание времени (напр. `7:00 MSK`)
7. Замените `/usr/bin/claude` на путь к Claude CLI (напр. `/opt/homebrew/bin/claude`)
